...
# pre: image
# Post: Faces are detected with colors
# we start by converting the parameter image to grey image and finding contours or "shapes"
# we use that to find edges and the image gets sharpened and sets thresholds.
# then for each contour we find the area and set a range
# we find the perimeter and then the area then find the difference between the original area and set a difference threshold.
# if there are 9 of those contours then and only then we find the colors of those contours in RGB values values are trial and error for the RGB ranges.
# then return faces and blob colors, faces are stored as numbers eg. #1 for white
def detect_face(bgr_image_input):

    # Converts image to grey, BGR color format to Grey
    gray = cv2.cvtColor(bgr_image_input,cv2.COLOR_BGR2GRAY)
    # kernel is a array of 1 and 0s in the shape of a ellipse
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(2,2))
    # takes gray and then using dilationa dn erosion it basically morphs the image to change the data into better "shapes"
    # Morph_open is bacially erosion followed by dilation
    gray = cv2.morphologyEx(gray, cv2.MORPH_OPEN, kernel)
    # dilation followed by erosion
    gray = cv2.morphologyEx(gray, cv2.MORPH_CLOSE, kernel)

    # transforms greyscale image to binary
    gray = cv2.adaptiveThreshold(gray,20,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,5,0)
    #cv2.imwrite()
    try:
        # parm : image, countours, Hierarchy
        # retrieves contours from binary image using an algo.
        # retr_ccomp retrieves all contours and organizes then in 2 levels, top lvl external boundaries, 2nd level boundaries of holes
    ...
    for contour in contours:
        # getting countour areas
        A1 = cv2.contourArea(contour)
        contour_id = contour_id + 1
        # if area is in a rance
...
# pre: takes the video
# post: Finds faces
# video stream, writer, up face, right face...
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            # if cube_solved == solver() then "Solved output"
